...
from .tasks import (create_task_list, create_task, get_tasks, get_task_lists, update_task_list, update_task,
                    delete_task_list, delete_task, is_admin, admin_get_task_lists, admin_get_tasks)
import jwt
...
class HTTPException(Exception):
    def __init__(self, status_code, detail="Something went wrong"):
...
class RequestParser:
    ...
    def parse(self) -> Response:
        ...
        if self.method == 'POST' and self.path == '/auth/reg':
            ...
            except (KeyError, ExpiredSignatureError, InvalidSignatureError) as e:
                return Response(status_code=404, body="Unauthorized")

            if is_admin(user_id) and self.method == 'GET' and self.path == '/task_list':
                return admin_get_task_lists()
            elif is_admin(user_id) and self.method == 'GET' and self.path == '/task':
                return admin_get_tasks(self.body['task_list_name'])

            if self.method == 'POST' and self.path == '/task_list':
                ...
                return update_task_list(self.body['task_list_old_name'], self.body['task_list_new_name'], user_id)
            elif self.method == 'DELETE' and self.path == '/task_list':
                return delete_task_list(self.body['task_list_name'], user_id)

            elif self.method == 'POST' and self.path == '/task':
                return create_task(self.body['task_list_name'], self.body['task_name'],
                                   self.body['description'], user_id)
                ...
                return update_task(self.body['task_old_name'], self.body['task_new_name'], self.body['new_desc'],
                                   self.body['task_list_name'], user_id)
            elif self.method == 'DELETE' and self.path == '/task':
                return delete_task(self.body['task_name'], self.body['task_list_name'], user_id)
